...
main_grp.add_argument('-o', '--output-directory', help = '<OUTPUT_DIRECTORY> (optional): output directory (default \'./downloaded_pictures/\')', default="downloaded_pictures")
...
main_grp.add_argument('-v', '--extract-val', help = '<EXTRACT_VALUE> (MANDATORY): which value to extract', type=str)
...
def extract_title(key_val):
    key, val = key_val

    extracted = val[options.extract_val]
...
def main():
    create_folder(options.output_directory)

    with open(path.join(options.input_file), "r") as f:
        datastore: dict = json.load(f)
        links_list = [(key, val) for key, val in sorted(datastore.items())]
        bulk_extract(links_list)
